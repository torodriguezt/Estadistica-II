...
Considere el siguiente conjunto de datos, una muestra de los *nacidos vivos registrados en el Hospital Manuel Uribe Ángel* (Envigado, Antioquia), cuyas madres residen en el Valle de Aburrá (Itagüí, Medellín, Envigado, Sabaneta, entre otros). Para facilitar el análisis en clase, la base original fue simplificada: se tomó una muestra reducida y se construyeron variables binarias, cuantitativas y categóricas a partir de la información cruda. La fuente original puede consultarse en el portal de *Datos Abiertos Colombia*.

Las variables consideradas son:

\begin{itemize}
  \item \textbf{BAJO\_PESO} (respuesta): vale $1$ si el recién nacido tuvo \textit{bajo peso al nacer} (menos de $2500$ g) y $0$ en caso contrario.
  \item \textbf{EDAD\_MADRE} (cuantitativa): edad de la madre, en años cumplidos.
  \item \textbf{CONSULTAS} (cuantitativa): número de consultas o controles prenatales realizados durante el embarazo.
...
\item Provea una \textit{interpretación directa} de los parámetros del modelo de regresión logística ajustado, así como una \textit{interpretación en términos de la razón de probabilidades}. Incluya coeficientes asociados a covariables cuantitativas (por ejemplo \textbf{EDAD\_MADRE}, \textbf{CONSULTAS}), binarias (\textbf{GEMELAR}) y categóricas (\textbf{REGIMEN}, respecto al nivel de referencia \textit{Contributivo}).
\end{enumerate}

```{=html}
<!--
# Apéndice: cómo se construyó la base simplificada
# A partir de la base 'Nacidos Vivos - Hospital Manuel Uribe Angel'
# (Datos Abiertos Colombia, recurso udqu-ifxr), descargada vía API:
#   - BAJO_PESO  <- peso_gramos (< 2500 g = 1; >= 2500 g = 0)
#   - EDAD_MADRE <- edad_madre (años)
#   - CONSULTAS  <- n_mero_consultas_prenatales
#   - GEMELAR    <- multiplicidad_embarazo (Doble/Triple = 1; Simple = 0)
#   - REGIMEN    <- r_gimen_seguridad (Contributivo / Subsidiado / NoAsegurado)
# Ver el script 'limpieza_nacimientos.py' (muestra de 2000 filas, semilla 2025).
